...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    newSpiral: function newSpiral() {
      var _this = this;

      var charset = "abcdefghijklmnopqrstuvwxyz1234567890";
      var charsLength = charset.length;
      var num1 = Math.round(Math.random() * (10 - 5) + 5);
      var num2 = Math.round(Math.random() * (10 - 5) + 5);
      ...
      for (var x = 0; x < num1; x++) {
        ...
        for (var y = 0; y < 5; y++) {
          data += charset.charAt(Math.floor(Math.random() * charsLength));
        }

        this.randoms.id.push(data);

        for (var a = 0; a < num2; a++) {
          var data2 = ""; //generate 5 character string

          for (var b = 0; b < 5; b++) {
            data2 += charset.charAt(Math.floor(Math.random() * charsLength));
          }
      ...
      this.axios.post("/api/NewRandoms", this.randoms).then(function (response) {
        if (response.data == "success") {
          _this.newBreakDowns();
        }
      });
    },
    newBreakDowns: function newBreakDowns() {
      var _this2 = this;

      this.axios.post("/api/NewBreakDowns", this.breakdowns).then(function (response) {
        if (response.data == "success") {
          _this2.GetSpiral();
        }
      });
    ...
    GetSpiral: function GetSpiral() {
      var _this3 = this;

      this.axios.get("/api/GetSpiral").then(function (response) {
        _this3.newRandomSet = response.data;

        _this3.showSpiral();
      });
    },
    showSpiral: function showSpiral() {
      this.display = "";

      for (var x = 0; x < this.newRandomSet.length; x++) {
        this.display += this.newRandomSet[x];

        for (var y = 0; y < this.newBreakdownSet.length; y++) {
          if (this.newRandomSet[x].id == this.newBreakdownSet[y].random_id) {
...
var staticRenderFns = [
  function() {
    ...
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "spiral-container" }, [
      _c("p", { attrs: { id: "spiral" } })
    ])
  }
]
...
__webpack_require__(/*! C:\Users\jhayngalang\Desktop\GalangSpiral\resources\js\app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! C:\Users\jhayngalang\Desktop\GalangSpiral\resources\css\app.css */"./resources/css/app.css");
